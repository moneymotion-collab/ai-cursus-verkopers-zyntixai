...
const tryProvisionAndLandMock = vi.hoisted(() => vi.fn());
...
vi.mock("@/features/auth/server/resolve-registration-destination", () => ({
  tryProvisionAndLand: tryProvisionAndLandMock,
}));
...
describe("auth callback route", () => {
  beforeEach(() => {
    ...
    tryProvisionAndLandMock.mockReset();
  ...
  it("lands recovery sessions on reset-password without provisioning", async () => {
    ...
    expect(tryProvisionAndLandMock).not.toHaveBeenCalled();
    expect(response.headers.get("location")).toBe(
      "http://localhost:3000/reset-password",
    );
  });

  it("rejects unsafe next destinations and falls back to provisioning path", async () => {
    exchangeCodeForSessionMock.mockResolvedValue({ error: null });
    getUserMock.mockResolvedValue({
      data: { user: { id: "user-1", email_confirmed_at: "2026-01-01T00:00:00Z" } },
      error: null,
    });
    tryProvisionAndLandMock.mockResolvedValue({
      ok: true,
    ...
    expect(tryProvisionAndLandMock).toHaveBeenCalled();
    expect(response.headers.get("location")).toBe(
      "http://localhost:3000/leads?org=11111111-1111-4111-8111-111111111111",
    );
  });

  it("accepts safe product next destinations after provisioning", async () => {
    exchangeCodeForSessionMock.mockResolvedValue({ error: null });
    getUserMock.mockResolvedValue({
      data: { user: { id: "user-1", email_confirmed_at: "2026-01-01T00:00:00Z" } },
      error: null,
