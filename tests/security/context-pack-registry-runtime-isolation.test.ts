...
const CONTEXT_TOKEN =
  /\bpublic\.context_packs\b|\bcontext_pack_versions\b|\bcontext_capability_mappings\b|\bcontext_terminology\b|\bcontext_pack_readiness\b|\bpack_key\b/;
...
function collectHits(paths: string[]): string[] {
  ...
  for (const filePath of paths) {
    ...
    if (CONTEXT_TOKEN.test(contents)) {
...
describe("CTX-1B context pack runtime isolation", () => {
  ...
  it("leaves protected Closed Beta surfaces free of Context identifiers", () => {
    const files: string[] = [];
  ...
  it("does not add Context FKs onto organizations in context migrations", () => {
    const migrationsDir = join(process.cwd(), "supabase/migrations");
    const hits: string[] = [];
    for (const name of readdirSync(migrationsDir)) {
      if (!name.includes("context_pack") && !name.includes("context-pack")) {
        continue;
      }
  ...
  it("does not change TAX-1, CAP-1, Social entitlement, or execution-gate identifiers", () => {
    ...
    expect(combined).not.toContain("insert into public.capabilities");
    expect(combined).not.toContain("update public.capabilities");
    expect(combined).not.toContain("insert into public.capability_dependencies");
    expect(combined).not.toContain("insert into public.capability_readiness");
