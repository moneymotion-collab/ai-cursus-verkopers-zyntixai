-- ZyntixAI ORG-CONTEXT-1C — guarded platform mutation RPC.
--
-- Additive function only. No table redesign. No backfill. No Production apply in 1C.
-- Does not alter organizations, TAX, CAP, CTX, onboarding, or Social.
-- PostgREST-callable so service-role application code can mutate atomically.
-- EXECUTE is revoked from public/anon/authenticated and granted only to service_role.
-- A public function here is not tenant self-service.
...
begin
  ...
  if p_actor_user_id is null then
    return jsonb_build_object(
      'ok', false,
      'code', 'UNAUTHORIZED',
      'message', 'Authenticated platform operator identity is required'
    );
  end if;
  ...
  return jsonb_build_object(
    'ok', false,
    'code', 'MUTATION_FAILED',
    'message', 'ORG-CONTEXT platform mutation was not applied'
  );
exception
...
comment on function public.apply_organization_context_platform_mutation(text, uuid, uuid, jsonb) is
  'ORG-CONTEXT-1C atomic platform mutation. Not tenant self-service, resolver, entitlement, or Social execution. Unapplied in 1C.';
