-- ZyntixAI BQA-1D — Qualification + classification atomic mutation RPC.
--
-- Additive function only. Does not alter BQA-1C tables.
...
-- Does not write support, admission, or demand tables.
-- EXECUTE is revoked from public/anon/authenticated and granted only to
-- the privileged database role. Caller authorization is established in
-- application server code before this function is invoked.
...
comment on function public.apply_business_qualification_mutation(text, uuid, uuid, uuid, uuid, jsonb) is
  'BQA-1D atomic qualification/classification mutations. Privileged execute only. Not caller authorization. Does not mutate Activity or Context assignment.';
