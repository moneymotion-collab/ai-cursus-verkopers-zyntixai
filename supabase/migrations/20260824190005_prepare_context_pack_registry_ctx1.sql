-- ZyntixAI DB-MIG-CTX1-C — prepare the CTX-1 child-protection trigger.
--
-- The frozen CTX-1 seed writes draft terminology before the later governed
-- CTX-1FV-R1C repair executes. Install those same repair semantics here so a
-- chronological migration replay can reach the seed without rewriting history.
--
-- Function replacement only. No trigger recreation, table DDL, or data DML.
...
begin
  ...
  if n_fn <> 1 then
    raise exception
      'DB-MIG-CTX1-C: expected public.context_pack_version_protect_children() trigger function once, found %',
      n_fn;
  end if;
  ...
  if n_map_trigger <> 1 then
    raise exception
      'DB-MIG-CTX1-C: expected context_capability_mappings_protect_children once, found %',
      n_map_trigger;
  end if;
  ...
  if n_term_trigger <> 1 then
    raise exception
      'DB-MIG-CTX1-C: expected context_terminology_protect_children once, found %',
      n_term_trigger;
  end if;
end $$;
