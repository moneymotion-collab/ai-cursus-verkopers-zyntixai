-- SMM-B1.11-E PRE-LIVE — scheduler_start consumes the matching one-shot window.
-- Fail-closed without a window unless unrestricted GUC is set.
-- Does not grant private.claim_due_social_publications.
...
comment on function public.scheduler_start_scheduled_publication_attempt(uuid, uuid) is
  'B1.11-E scheduler claim+start. service_role only. Consumes the matching controlled window before claim. Fail-closed without a window unless unrestricted GUC is set. Does not grant private.claim_due_social_publications.';
