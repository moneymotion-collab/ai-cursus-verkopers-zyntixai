-- ZyntixAI DATA-1F-R1 — Allow mapping-state DATA intake session cancellation.
--
-- Smallest exact fix: existing cancel_session operation now accepts
-- mapping_required and mapped in addition to created, source_ready, parsed.
-- The DATA-1B/1C status graph already allowed those edges. The foundation
-- RPC allowlist did not. No new tables, RLS, grants, Storage, parser schema,
-- mapping catalog, staging, or Customer changes. Cancellation retains
-- mapping rows, confirmation evidence, and immutable events.
...
begin
  ...
  if p_operation = 'cancel_session' then
    if v_session.status not in ('created', 'source_ready', 'parsed', 'mapping_required', 'mapped') then
      return jsonb_build_object(
        'ok', false,
        'code', 'INVALID_STATE',
        'message', 'DATA can cancel only created, source_ready, parsed, mapping_required, or mapped sessions'
      );
    end if;
...
comment on function public.apply_data_intake_foundation_mutation(text, uuid, uuid, uuid, jsonb) is
  'DATA-1C/1E-R1/1F-R1 service_role-only foundation mutations. Human Owner/Admin identity is required. cancel_session allows created, source_ready, parsed, mapping_required, and mapped. No staging. No import.';
