-- ZyntixAI DATA-1C — RLS, grants, and foundation mutation RPC.
--
-- Authenticated/anon/public have no SELECT/INSERT/UPDATE/DELETE on DATA tables.
-- Owner/Admin access is a governed server path, not direct PostgREST.
-- service_role is the database executor, never the human actor.
-- EXECUTE on the mutation RPC is service_role only.

alter table public.data_intake_sessions enable row level security;
alter table public.data_intake_sources enable row level security;
alter table public.data_intake_mappings enable row level security;
alter table public.data_intake_staging_rows enable row level security;
alter table public.data_import_plans enable row level security;
alter table public.data_intake_events enable row level security;
alter table public.data_external_record_links enable row level security;
alter table public.data_import_row_results enable row level security;

revoke all on table public.data_intake_sessions from public;
revoke all on table public.data_intake_sessions from anon;
revoke all on table public.data_intake_sessions from authenticated;
revoke all on table public.data_intake_sessions from service_role;

revoke all on table public.data_intake_sources from public;
revoke all on table public.data_intake_sources from anon;
revoke all on table public.data_intake_sources from authenticated;
revoke all on table public.data_intake_sources from service_role;

revoke all on table public.data_intake_mappings from public;
revoke all on table public.data_intake_mappings from anon;
revoke all on table public.data_intake_mappings from authenticated;
revoke all on table public.data_intake_mappings from service_role;

revoke all on table public.data_intake_staging_rows from public;
revoke all on table public.data_intake_staging_rows from anon;
revoke all on table public.data_intake_staging_rows from authenticated;
revoke all on table public.data_intake_staging_rows from service_role;

revoke all on table public.data_import_plans from public;
revoke all on table public.data_import_plans from anon;
revoke all on table public.data_import_plans from authenticated;
revoke all on table public.data_import_plans from service_role;

revoke all on table public.data_intake_events from public;
revoke all on table public.data_intake_events from anon;
revoke all on table public.data_intake_events from authenticated;
revoke all on table public.data_intake_events from service_role;

revoke all on table public.data_external_record_links from public;
revoke all on table public.data_external_record_links from anon;
revoke all on table public.data_external_record_links from authenticated;
revoke all on table public.data_external_record_links from service_role;

revoke all on table public.data_import_row_results from public;
revoke all on table public.data_import_row_results from anon;
revoke all on table public.data_import_row_results from authenticated;
revoke all on table public.data_import_row_results from service_role;

grant select, insert, update on table public.data_intake_sessions to service_role;
grant select, insert, update on table public.data_intake_sources to service_role;
grant select, insert, update on table public.data_intake_mappings to service_role;
grant select, insert, update, delete on table public.data_intake_staging_rows to service_role;
grant select, insert, update on table public.data_import_plans to service_role;
grant select, insert on table public.data_intake_events to service_role;
grant select, insert, update on table public.data_external_record_links to service_role;
grant select, insert, update on table public.data_import_row_results to service_role;
  ...
  if p_operation = 'cancel_session' then
    if v_session.status not in ('created', 'source_ready') then
      return jsonb_build_object(
        'ok', false,
        'code', 'INVALID_STATE',
        'message', 'DATA-1C can cancel only pre-import created or source_ready sessions'
      );
    end if;
...
comment on function public.apply_data_intake_foundation_mutation(text, uuid, uuid, uuid, jsonb) is
  'DATA-1C service_role-only foundation mutations. Human Owner/Admin identity is required. No parser. No import.';
