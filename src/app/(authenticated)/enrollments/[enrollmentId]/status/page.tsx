...
import { loadEnrollmentStatusPage } from "@/features/enrollments/ui/load-enrollment-lifecycle-workflow-page";
import { buildBackToEnrollmentsHref } from "@/features/enrollments/ui/enrollment-navigation";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import styles from "../../page.module.css";

export const metadata = authenticatedPageMetadata("/enrollments/[enrollmentId]/status");


type EnrollmentStatusPageProps = {
...
export default async function EnrollmentStatusPage({
  ...
  const result = await loadEnrollmentStatusPage(supabase, enrollmentId, rawSearchParams);
    ...
    return (
      <AppShell activeNav="enrollments">
        <section className={styles.statePanel} aria-labelledby="auth-required-title">
          ...
          <p>Please sign in to change enrollment status.</p>
        </section>
      </AppShell>
    );
  }
    ...
    return (
      <AppShell activeNav="enrollments" organizationOptions={result.organizations}>
        <EnrollmentOrganizationRequiredPanel
          ...
          targetPath={`/enrollments/${enrollmentId}/status`}
          description="Select an organization before changing this enrollment status."
        />
      </AppShell>
    );
  }
  ...
  return (
    <AppShell
      ...
      organizationSelectorAction={`/enrollments/${enrollmentId}/status`}
    >
