...
import { loadEnrollmentArchivePage } from "@/features/enrollments/ui/load-enrollment-lifecycle-workflow-page";
import { buildBackToEnrollmentsHref } from "@/features/enrollments/ui/enrollment-navigation";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import styles from "../../page.module.css";

export const metadata = authenticatedPageMetadata("/enrollments/[enrollmentId]/archive");


type EnrollmentArchivePageProps = {
  params: Promise<{ enrollmentId: string }>;
  searchParams: Promise<Record<string, string | string[] | undefined>>;
};

export default async function EnrollmentArchivePage({
  params,
  searchParams,
}: EnrollmentArchivePageProps) {
  const supabase = await createSupabaseServerClient();
  const { enrollmentId } = await params;
  const rawSearchParams = await searchParams;
  const result = await loadEnrollmentArchivePage(supabase, enrollmentId, rawSearchParams);
    ...
    return (
      <AppShell activeNav="enrollments">
        <section className={styles.statePanel} aria-labelledby="auth-required-title">
          ...
          <p>Please sign in to archive enrollments.</p>
        </section>
      </AppShell>
    );
  }
    ...
    return (
      <AppShell activeNav="enrollments" organizationOptions={result.organizations}>
        <EnrollmentOrganizationRequiredPanel
          ...
          targetPath={`/enrollments/${enrollmentId}/archive`}
          description="Select an organization before archiving this enrollment."
        />
      </AppShell>
    );
  }
  ...
  if (result.kind === "action_unavailable") {
    return (
      <AppShell activeNav="enrollments">
        <section className={styles.statePanel}>
          <h1>Archive enrollment unavailable</h1>
          <p>{result.message}</p>
  ...
  return (
    <AppShell
      ...
      organizationSelectorAction={`/enrollments/${enrollmentId}/archive`}
    >
