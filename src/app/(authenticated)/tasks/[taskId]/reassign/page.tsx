...
import { loadTaskReassignPage } from "@/features/tasks/ui/load-task-workflow-page";
...
export const metadata = authenticatedPageMetadata("/tasks/[taskId]/reassign");


type TaskReassignPageProps = {
  params: Promise<{ taskId: string }>;
  searchParams: Promise<Record<string, string | string[] | undefined>>;
};

export default async function TaskReassignPage({ params, searchParams }: TaskReassignPageProps) {
  const supabase = await createSupabaseServerClient();
  const { taskId } = await params;
  const rawSearchParams = await searchParams;
  const result = await loadTaskReassignPage(supabase, taskId, rawSearchParams);
    ...
    return (
      <AppShell organizationOptions={result.organizations}>
        <OrganizationRequiredPanel
          ...
          targetPath={`/tasks/${taskId}/reassign`}
        />
      </AppShell>
    );
  }
  ...
  if (result.kind === "action_unavailable") {
    return (
      <AppShell>
        <section className={styles.statePanel}>
          <h1>Reassign unavailable</h1>
          <p>This task cannot be reassigned in its current state.</p>
