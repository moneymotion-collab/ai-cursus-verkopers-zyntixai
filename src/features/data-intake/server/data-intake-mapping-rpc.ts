...
import type { DataIntakeMappingOperation } from "@/features/data-intake/domain/authorization";
import type { Database } from "@/types/database";

export const DATA_INTAKE_MAPPING_RPC =
  "apply_data_intake_mapping_mutation" as const satisfies keyof Database["public"]["Functions"];

type DataIntakeMappingFn = Database["public"]["Functions"][typeof DATA_INTAKE_MAPPING_RPC];

export type DataIntakeMappingRpcArgs = Omit<
  DataIntakeMappingFn["Args"],
  "p_operation" | "p_payload"
> & {
  p_operation: DataIntakeMappingOperation;
  p_payload: Record<string, unknown>;
};

export type DataIntakeMappingRpcClient = {
  rpc(
    fn: typeof DATA_INTAKE_MAPPING_RPC,
    args: DataIntakeMappingRpcArgs,
  ): PromiseLike<{
    data: DataIntakeMappingFn["Returns"];
...
export function mapDataIntakeMappingRpcPayload(
  payload: unknown,
...
export async function invokeDataIntakeMappingMutation(
  client: DataIntakeMappingRpcClient,
  args: DataIntakeMappingRpcArgs,
): Promise<DataIntakeResult<DataIntakeFoundationSuccess>> {
  const { data, error } = await client.rpc(DATA_INTAKE_MAPPING_RPC, args);
  if (error) {
    return dataFail("DATABASE_WRITE_ERROR", error.message, {
      code: error.code ?? null,
    });
  }
  return mapDataIntakeMappingRpcPayload(data);
}
