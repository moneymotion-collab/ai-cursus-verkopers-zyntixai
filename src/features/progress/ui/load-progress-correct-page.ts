...
const ACTION_UNAVAILABLE_MESSAGE =
  "This progress record cannot be corrected in its current state.";

/**
 * Reuses the B1.6.2 detail loader for org resolution, fact loading, and
 * permission-aware visibility, then gates access to the correction workflow.
 */
export async function loadProgressCorrectPage(
  supabase: SupabaseClient<Database>,
  factId: string,
  rawSearchParams: Record<string, string | string[] | undefined>,
): Promise<ProgressCorrectPageResult> {
  ...
  if (!canShowCorrectProgressWorkflow(result.capabilities)) {
