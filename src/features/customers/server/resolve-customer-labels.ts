...
export const PROGRAM_LABEL_UNAVAILABLE = "Unavailable program";
...
export async function resolveProgramLabels(
  supabase: SupabaseClient<Database>,
  organizationId: string,
  programIds: string[],
): Promise<Record<string, string>> {
  ...
  for (const program of programs ?? []) {
    labels[program.id] = normalizeLabel(program.name, PROGRAM_LABEL_UNAVAILABLE);
  }
...
export function resolveProgramLabel(
  ...
  return labels[programId] ?? PROGRAM_LABEL_UNAVAILABLE;
}
