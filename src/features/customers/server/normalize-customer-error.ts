...
import type {
  ...
  CustomerApplicationErrorCode,
} from "@/features/customers/domain/types";

type NormalizeOptions = {
  fallbackCode?: CustomerApplicationErrorCode;
};
...
const MESSAGE_RULES: Array<{
  ...
  message: string;
  retryable: boolean;
  category: CustomerApplicationError["category"];
}> = [
  {
  ...
  {
    pattern: /^insufficient role to create customers$/i,
  ...
  {
    pattern: /^insufficient role to transition customer status$/i,
    code: "INSUFFICIENT_ROLE",
    message: "You don't have permission for this action.",
    retryable: false,
    category: "permission",
  ...
  {
    pattern: /^insufficient role to archive customers$/i,
    code: "INSUFFICIENT_ROLE",
    message: "You don't have permission for this action.",
    retryable: false,
    category: "permission",
  ...
  {
    pattern: /^insufficient role to restore customers$/i,
    code: "INSUFFICIENT_ROLE",
    message: "You don't have permission for this action.",
    retryable: false,
    category: "permission",
  ...
  {
    pattern: /^customer email already exists in organization$/i,
  ...
  {
    pattern: /^customer not found or already archived$/i,
    code: "CUSTOMER_UNAVAILABLE",
    message: "Customer not found or access denied.",
    retryable: false,
    category: "not_found",
  ...
  {
    pattern: /^customer not found or not archived$/i,
    code: "CUSTOMER_UNAVAILABLE",
    message: "Customer not found or access denied.",
    retryable: false,
    category: "not_found",
  ...
  {
    pattern: /^archived customers cannot transition status$/i,
    code: "ARCHIVED_RECORD",
    message: "Archived customers cannot be changed.",
    retryable: false,
    category: "conflict",
  },
  {
    ...
    code: "INVALID_STATE",
    message: "This customer changed and must be reloaded.",
    retryable: false,
    category: "conflict",
  },
  {
...
function mapTransportError(error: PostgrestError | Error): CustomerApplicationError {
...
export function normalizeCustomerError(
  error: unknown,
  ...
  if (error && typeof error === "object" && ("code" in error || "details" in error || "status" in error)) {
...
export function invalidInputError(fieldErrors?: Record<string, string>): CustomerApplicationError {
  return {
    code: "INVALID_INPUT",
    message: "Please check the highlighted fields.",
    retryable: false,
...
export function insufficientRoleError(): CustomerApplicationError {
  return {
    code: "INSUFFICIENT_ROLE",
    message: "You don't have permission for this action.",
    retryable: false,
...
export function archivedRecordError(): CustomerApplicationError {
  return {
    code: "ARCHIVED_RECORD",
    message: "Archived customers cannot be changed.",
    retryable: false,
...
export function invalidOwnerError(): CustomerApplicationError {
  return {
    code: "INVALID_OWNER",
    message: "Selected owner is not valid for this organization.",
    retryable: false,
...
export function mutationCommittedRefreshRequiredError(): CustomerApplicationError {
  return {
...
export function permissionDeniedError(): CustomerApplicationError {
  return {
...
export function orgContextMissingError(): CustomerApplicationError {
  return {
    code: "ORG_CONTEXT_MISSING",
    message: "Organization not found or access denied.",
    retryable: false,
...
export function authRequiredError(): CustomerApplicationError {
  return {
    code: "AUTH_REQUIRED",
    message: "Please sign in to continue.",
    retryable: false,
...
export function customerUnavailableError(): CustomerApplicationError {
  return {
    code: "CUSTOMER_UNAVAILABLE",
    message: "Customer not found or access denied.",
    retryable: false,
...
export function mapOrganizationContextError(
  ...
  switch (error.code) {
    ...
    default:
      return normalizeCustomerError(error.message);
  }
}
